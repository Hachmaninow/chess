...
(defn- ext-piece [str index] (keyword (subs str index (inc index))))
...
; TODO: This seems super ugly. I guess it was done because Instaparse is not available in Clojurescript?
(defn parse-move
  ...
  (let [s (name input)]
    (cond
      ...
      (re-matches #"[a-h][1-8]=[N|B|R|Q|K]" s) {:piece :P :to (ext-sqr s 0) :promote-to (ext-piece s 3)}
      (re-matches #"[N|B|R|Q|K].." s) {:piece (ext-piece s 0) :to (ext-sqr s 1)}
      (re-matches #"[a-h]x[a-h][1-8]" s) {:piece :P :from-file (ext-file s 0) :capture :X :to (ext-sqr s 2)}
      (re-matches #"[a-h]x[a-h][1-8]=[N|B|R|Q|K]" s) {:piece :P :from-file (ext-file s 0) :capture :X :to (ext-sqr s 2) :promote-to (ext-piece s 5)}
      (re-matches #"[N|B|R|Q|K]x[a-h][1-8]" s) {:piece (ext-piece s 0) :capture :X :to (ext-sqr s 2)}
      (re-matches #"[N|B|R|Q|K][a-h][a-h][1-8]" s) {:piece (ext-piece s 0) :from-file (ext-file s 1) :to (ext-sqr s 2)}
      (re-matches #"[N|B|R|Q|K][1-8][a-h][1-8]" s) {:piece (ext-piece s 0) :from-rank (ext-rank s 1) :to (ext-sqr s 2)}
      (re-matches #"[N|B|R|Q|K][a-h]x[a-h][1-8]" s) {:piece (ext-piece s 0) :from-file (ext-file s 1) :capture :X :to (ext-sqr s 3)}
      (re-matches #"[N|B|R|Q|K][1-8]x[a-h][1-8]" s) {:piece (ext-piece s 0) :from-rank (ext-rank s 1) :capture :X :to (ext-sqr s 3)}
      :default (throw (ex-info (str "Could not parse move: " input) {:input s}))
      )))
